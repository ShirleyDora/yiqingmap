function getData() {
    // let str = localStorage.getItem('data');E
    ...
    data.newslist.forEach(item => {
        arr.push({
            ...
            current: item.confirmedCount, // 累计确诊
...
var option = {
    ...
    visualMap: [{
        ...
        pieces: [
            { gt: 100000 },
            { gt: 20000, lte: 100000 }, // 不指定 max，表示 max 为无限大（Infinity）。
            { gt: 4000, lte: 20000 },
            { gt: 800, lte: 4000 },
            { gt: 160, lte: 800 },
            { gt: 40, lte: 160 },
            { gt: 0, lte: 40 }     // 不指定 min，表示 min 为无限大（-Infinity）。
        ],
        textStyle:{
            color:'white'
        },
        inRange: {
            color: ['#FCDBEC', '#FFE6C5','#ECD1F0','#A4D3D2','#88C9AB','#FDEF73','#F8AF5E']
        },
        // showLabel: {
        //     show: true
        // }
